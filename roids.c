...
void ship_draw( const ship_t* ship )
{
  ...
  */
	...
	for(wx=-1;wx<2;wx++){
    	for(wy=-1;wy<2;wy++){
        	draw_triangle( ship->x +wx+ dx, ship->y +wy+ dy,ship->x +wx+ lx, ship->y +wy+ ly,ship->x +wx+ rx, ship->y +wy+ ry,shipcolor);
        	if( ship->thrust ){
				draw_triangle( ship->x+wx - dx, ship->y+wy - dy,ship->x +wx + lx, ship->y +wy+ ly,ship->x +wx+ rx, ship->y +wy+ ry,thrustcolor );
           }
       }
	}
}
...
void roid_draw( const roid_t* roid )
{
  ...
  for (wx=-1; wx<2; wx++){
    for (wy=-1; wy<2; wy++){
      draw_rectangle( roid->x+wx-roid->width/2.0, roid->y+wy-roid->height/2.0, 
		  roid->x+wx+roid->width/2.0, roid->y+wy+roid->height/2,
		  roid->color );
    }
  }
...
int shot_roid_hit( const shot_t* shot, const roid_t* roid )
{
   ...
   */
	...
	for (wx=-1; wx<2; wx++){
		for (wy=-1; wy<2; wy++){
			if (shot->x+wx >= roid->x - roid->width/2 &&
	  		shot->x+wx <= roid->x + roid->width/2 &&
	  		shot->y+wy >= roid->y - roid->height/2 &&
	  		shot->y+wy <= roid->y + roid->height/2 ){
				return 1;
			}
		}
